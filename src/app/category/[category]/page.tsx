...
const CategoryPage = () => {
  ...
  useEffect(() => {
    if (category) {
      ...
      const decodedCategory = decodeURIComponent(formattedCategory);
      let apiUrl = `http://127.0.0.1:8000/api/products/category/${decodedCategory}/`;

      if(decodedCategory === "all"){
        apiUrl = "http://127.0.0.1:8000/api/products/"; 
      }else if (decodedCategory.toLowerCase() === "brands"){
        apiUrl = "http://127.0.0.1:8000/api/brands/";
      }
      fetch(apiUrl)
      .then((res) => res.json())
      .then((data) => {
        if (decodedCategory.toLowerCase() === "brands") {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <div key={product.id} className="shadow-md rounded-lg overflow-hidden">
              ...
              <Link  href={`/product/${product?.id}`}  className=" bg-red-500 text-white hover:bg-white hover:text-red-500 w-full h-10 flex items-center justify-center">Ürün detayina git</Link>
              </div>
            </div>
          ))}
        </div>
